...
fn test_solidity_compile() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("compile").arg("solidity").arg(format!("{contract_path}.sol"));
    cmd.assert().success().stdout(predicate::str::contains(
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("compile").arg("solidity").arg(format!("{contract_path}.sol")).arg("--function-ids");
    cmd.assert().success().stdout(predicate::str::contains(r#""add": "0x25efb208","#));
    ...
    cmd.assert().success().stdout(predicate::str::contains(r#""id": 3297096926,"#));
